...
- Current Stream Sort build: beta `0.3.6-beta.6` from source commit `ab9dfd3c28afe61f757cfb6a40cd0217760f09f3`; the immutable tag archive preserves `stream_sorter/plugin.json`, compiles, and passes all 107 source tests. The managed upgrade required one Dispatcharr container restart to load the new Python action dispatcher. After restart, a serial cancellation checkpointed 3 completed media checks, ran 0 throughput checks, recorded 0 capacity deferrals, left the remaining 7 streams unprobed, exited as canceled, restored production settings, and left scheduling disabled.
