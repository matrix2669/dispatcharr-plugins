...
Stable registry metadata reused by the development channel for the Dispatcharr Arr Stack Plugin.
...
Version `0.2.0` changes the Dispatcharr plugin identity. Existing installations must disable the old plugin, move `/data/plugins/dispatcharr_vod_newznab` to `/data/plugins/arr_stack_connector`, move `/data/dispatcharr_vod_newznab` to `/data/arr_stack_connector`, and copy the old plugin settings into the renamed plugin record. Do not enable both identities at once because both use port `9192` by default.
